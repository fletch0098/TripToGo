 ...
 */
?>
<article id="post-<?php the_ID(); ?>" <?php ( is_sticky() && is_home() && ! is_paged() ) ? post_class( 'card card-raised card-blog' ): post_class( 'card card-plain card-blog' ); ?>>
	<div class="row alternative-blog-row">
		...
		<div class="col-md-7">
			...
			<div class="col-md-12">
				...
				<?php the_title( sprintf( '<h2 class="card-title"><a href="%s" title="%s" rel="bookmark">', esc_url( get_permalink() ), the_title_attribute( array(
					'echo' => false,
				) ) ), '</a></h2>' ); ?>
				...
				<div class="author">
					...
					printf( esc_html__( 'By %1$s, %2$s', 'hestia-pro' ),
						/* translators: %1$s is Author name, %2$s is author link */
						sprintf( '<a href="%2$s" title="%1$s"><b>%1$s</b></a>',
							esc_html( get_the_author() ),
							esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) )
						),
						/* translators: %1$s is Time since post, %2$s is author Close tag */
						sprintf( esc_html__( '%1$s ago %2$s','hestia-pro' ),
							/* translators: %1$s is Time since, %2$s is Link to post */
							sprintf( '<a href="%2$s"><time>%1$s</time>',
