...
function hestia_woocommerce_header_add_to_cart_fragment( $fragments ) {
	...
	<a class="cart-contents btn btn-white pull-right" href="<?php echo esc_url( wc_get_cart_url() ); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'hestia-pro' ); ?>">
		...
		sprintf( _n( '%d item', '%d items', absint( $woocommerce->cart->cart_contents_count ), 'hestia-pro' ), absint( $woocommerce->cart->cart_contents_count ) );?> - <?php echo wp_kses( $woocommerce->cart->get_cart_total(), array(
			'span' => array(
			'class' => array(),
			),
		) ); ?></a>
