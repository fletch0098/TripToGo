...
class Hestia_Contact_Info extends WP_Customize_Control {
	...
	public function render_content() {
		if ( defined( 'PIRATE_FORMS_VERSION' ) ) {
			...
			printf( esc_html__( 'You should be able to see the form on your front-page. You can configure settings from %s, in your WordPress dashboard.', 'hestia-pro' ),
				/* translators: %s is Path in plugin*/
				sprintf( '<b>%s</b>', esc_html__( 'Settings > Pirate Forms', 'hestia-pro' ) )
			);
		} else {
			/* translators: %1$s is Plugin name */
			printf( esc_html__( 'In order to add a contact form to this section, you need to install the %s plugin.','hestia-pro' ),
